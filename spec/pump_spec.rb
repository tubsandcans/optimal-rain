...
describe "Pump" do
  ...
  before do
    TurnAndBurnRunner::Pump.insert(pin_number: 17, cycle_start: start_time)
  end

  context "when cycle-start is Time.now" do
    let(:pump) { TurnAndBurnRunner::Pump.last }
    ...
    it "schedules the next watering after the last scheduled job completes" do
      ...
      expect(TurnAndBurnRunner::ACTIVE_PINS[pump.pin_number].value).to eq 1
    ...
    it "will run next_watering() tomorrow at light-on time after today's last watering" do
      ...
      allow(Time).to receive(:now).and_return(
        TurnAndBurnRunner::ACTIVE_SCHEDULES[pump.pin_number].jobs.first.original
      )
  ...
  context "when cycle start was 65 days ago (cycle is completed)" do
    ...
    it "is all out of watering events" do
      ...
      pump.next_watering
      expect(TurnAndBurnRunner::ACTIVE_SCHEDULES[pump.pin_number]).to be_nil
    end
  end
end
